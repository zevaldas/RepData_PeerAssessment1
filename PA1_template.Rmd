...
**Total number of steps taken per day mean is `r format(snpd_mean)` and median is `r format(snpd_median)`.**
...
**`r int_number` 5-minute interval, on average across all the days in the dataset, contains the maximum number of steps.**
...
**Total number of steps taken per day (without missing date) mean is `r format(new_snpd_mean)` and median is `r format(new_snpd_median)`.**
